...
- **No company user can be invited before the company is active.** `POST /companies/:id/users` refuses with 409 unless `status = 'active'`, before any row is written, and rolls back. `pending`, `suspended` and `offboarded` are all refused, and because the direct invitation and the nomination approval share that one handler, one guard covers both. The rule is `canInviteUsers()` / `inviteRefusalMessage()` in `services/companies.js`, next to the activation gates. **Why:** the workspace does not exist until activation seeds it and `requireCompany` refuses a pending company outright, so an invitation issued early buys the invitee a sign-in, an MFA enrolment and an empty screen. Found on production during the CW004 smoke check; see `Code/Handover/HANDOVER-CW005-*.md`. Deliberately **not** a queue: there is no invite-now-send-later behaviour to get wrong.
...
- **A replaced version is retired, not deleted, and that is what clears a flag.** `file_status` carries `superseded` (migration 015, on its own for the 010 reason). `deriveItemState()` excludes a superseded file alongside a deleted one, so a version a newer one has overtaken stops counting towards its checklist item. Two paths set it: `POST /company/submit` retires a SUBMITTED predecessor of anything carrying `supersedes`, inside the submitting transaction; and a reviewer can set it by hand, which is the path that matters when the company sent its correction as a fresh document instead of up the version chain. **A company action can therefore clear a reviewer's `attention_needed`** — intended, because the replacement enters at `received` and the item lands back in the review queue rather than at `completed`. Nothing is deleted, in Postgres or in S3.
- **`company_irl_items.baseline_state` is the state an item falls back to, and it is stored because it can no longer be inferred.** `recomputeItemState()` used to guess the fallback from the item's *current* state, which only survived while the item had never moved. Superseding made items travel backwards for the first time, and the guess would have silently downgraded an item Taranis holds in full into one the company is asked for again. Migration 016 adds the column and backfills it with the same rule `seedStateFor()` applies; keep those two in step. Only `outstanding`, `partially_held` and `held` may be baselines — the derived states are a projection of the files, and `not_applicable` short circuits the recompute outright.
...
- **Email goes through the outbox, never inline.** Phase 1b (migrations 017/018) added `notification_outbox` and a `setInterval` worker in `services/notifications.js`. **Routes call `queue(client, …)` with the transaction's client and nothing else touches SES**, so a notification commits with the event it announces or rolls back with it, and no request ever waits on a third party. A send that succeeded inside a transaction that later rolled back could not be taken back, which is the whole reason for the pattern. `attempts` is incremented at CLAIM time, not on failure, so a message that crashes the worker cannot spin at the head of the queue.
- **There is no SES credential and none should ever be created.** `services/email.js` uses the SES v2 SDK on the task role via the container credential chain, exactly as the S3 client does (HANDOVER-C002 §5.2 supersedes the code brief §5). An authorisation failure in production is an IAM problem, never a missing secret.
- **The ten email templates are approved wording and frozen.** They live as literal strings in `services/email-templates/templates.js` and are asserted character for character in `test/email-templates.test.js`. A wording change comes back through Cowork as an amended approved file; it is not the code side's to make, however obvious the improvement looks. Eight are wired; `new-comment` and `reminder-outstanding` ship rendered and tested but unwired, because comments and scheduled reminders do not exist.
- **`PORTAL_URL` must be set on the task definition or the API refuses to start in production.** Every template links back to the portal, and a relative link in an email is a dead link. `services/links.js` holds the client route paths — if a route moves in `packages/web/src/App.jsx` it moves there too, because the portal keeps working when they drift and only the emails break, silently, for the counterparty.
- **The suppression list is fed by a queue that does not exist yet.** `services/ses-events.js` is built and tested but starts nothing until `SES_EVENTS_QUEUE_URL` is set; the configuration set, SNS topic and SQS queue are console work (HANDOVER-C011 §3.3). SES's own account-level suppression applies regardless, so the commitment made to AWS holds meanwhile. Hard bounces and complaints suppress; a **transient** bounce must not, because a full mailbox is what the retry is for.
- **`/health` reports `email`.** `"email": "log"` on a production task means invitations and receipts are being queued and printed rather than sent, which otherwise looks identical to working — the same reasoning as `storage` and `scanner`.
- **The audit log cannot be edited, and that decides more than it looks like it does.** The triggers in migration 004 raise on UPDATE and on DELETE unconditionally, and `audit_log.user_id` is a foreign key to `users(id)` with no ON DELETE clause. Together those mean **a user who has ever signed in cannot be deleted at all**: the FK blocks the delete, and the two ways round it (removing the audit rows, or nulling their user_id) are both blocked by the triggers. Anything that needs a user gone must retire the account in place instead — `status = 'disabled'`, memberships removed, `refresh_tokens` and `user_mfa` cleared. Do not reach for dropping a trigger; that is the DFSA commitment itself. `services/test-data-purge.js` is the worked example, and it lets the database decide per user by attempting the delete inside a SAVEPOINT.
- **`POST /maintenance/purge-test-data` can only ever touch four named companies.** The allowlist is compiled into `services/test-data-purge.js` and takes no parameter, the company must be `offboarded`, and destruction needs the exact phrase `PURGE TEST DATA`; anything else is a dry run. It is safe to leave deployed because there is no argument that widens it. It is deliberately **not** a migration — `autoMigrate()` runs on every deploy and a destructive step must never be something a redeploy repeats.
- **Fund documents are restored by a CLIENT, not a server route.** `tools/import-fund-documents.mjs` runs on the workstation that holds the files and posts them one at a time to the ordinary `POST /documents`, because nothing else can see both the files and the bucket: RDS is in a private subnet and the deploy credential is ECR and ECS only. Idempotence is a **skip, not an upsert** — `documents` has no natural key, since two rows with the same title is what a version looks like — so the client reads what is already there and does not re-send it. Start with `npm run import:fund-docs -- --list`, which prints the fund slugs and category names the manifest must use; there are **seven** categories, not the nine the older briefs say.
- **`npm run lint` is a real check again.** There was no ESLint config in either format from the moment ESLint 9 became the installed version, so both workspaces exited 2 without linting a line. `eslint.config.js` at the root covers both and is deliberately narrow: mistakes, not style. One known gap is written into it — web `no-unused-vars` ignores capitalised identifiers, because ESLint core does not count a JSX reference as a use and closing that properly means adding `eslint-plugin-react`.
...
11 August 2026 (CW012 pre-counterparty cleanup: the guarded smoke-test purge behind `/maintenance/purge-test-data`, the manifest-driven fund document importer, the two stale invite copy surfaces corrected now that Phase 1b sends, an invite modal that names an existing account before it is reused, plus the first pull-request CI workflow and a working ESLint 9 flat config — see `Code/Handover/HANDOVER-CW012-*.md` and the Code-side reply `HANDOVER-C012-*.md`).

Previously 10 August 2026 (DD Portal Phase 1b: SES on the task role, the `notification_outbox` and its worker, the suppression list, the ten approved templates as HTML and plain text, and eight per-event triggers wired into the existing routes — see `Code/Handover/HANDOVER-CW011-*.md` and the Code-side reply `HANDOVER-C011-*.md`).

Previously 7 August 2026 (superseded file versions and a stored item baseline: migrations 015 and 016, automatic retirement of a replaced predecessor at submission, a reviewer-settable Superseded status, one shared Set status option list across both admin modals, and the company item page's replace link made visible — see `Code/Handover/HANDOVER-CW010-*.md`).

Previously 6 August 2026 (Taranis-to-company shared documents: migration 014 `company_shared_files`, admin publish / list / soft-withdraw / read-back routes, a read-only company-portal list and download, a Shared documents tab on CompanyDetailPage and a From Taranis page in the company portal — see `Code/Handover/HANDOVER-C005-*.md` §3.5).
