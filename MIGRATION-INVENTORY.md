...
- **Head migration:** `014_company_shared_files.sql`.
- **Full migration list:**
  ...
  14. `014_company_shared_files.sql` — DD portal, Taranis-to-company shared documents (HANDOVER-C005 §3.5). `company_shared_files`: one row per document Taranis publishes into one company's workspace, read-only to that company. **Withdrawal is soft and the table is never deleted from** — `withdrawn_at` hides a row from the company while the row, the S3 object and the audit trail survive, so a document a counterparty downloaded before it was pulled is still accounted for. A CHECK enforces that `withdrawn_at` and `withdrawn_by` move together, so a withdrawal always names an actor. S3 keys use their own top-level prefix `taranis-shared/{companyId}/{sharedFileId}/{filename}`, disjoint from both `companies/` and `documents/`. The code brief §4 sketched this as `company_shared_documents`; it is `company_shared_files` to sit beside `company_files` and keep "documents" meaning fund documents.
- **Migration 014 verification, 2026-08-06:** same procedure as below. `001` to `014` applied in order to a throwaway PostgreSQL 16 container, one file per transaction, aborting on first failure; `014` then force-re-run twice and `010`–`014` re-run as a block, all no-ops. Constraints then exercised with real inserts against that container: a blank title refused, a withdrawal with no actor refused, clearing `withdrawn_by` on a withdrawn row refused, an unrecognised `scan_state` refused, and a properly attributed withdrawal accepted.
...
Three **latent bugs** were exposed during the rotation (all documented as Tier 3 pre-flight checks in `C:\Users\mark\Claude Cowork\Other\Admin\WORKFLOW.md` → Latent bugs exposed during Tier 3 migrations):
...
- **Per-package Dockerfiles used `npm ci` without a workspace lockfile** — monorepo uses npm workspaces with the authoritative `package-lock.json` at the repo root; `packages/api` and `packages/web` carry no lockfile of their own. `npm ci` fails on a fresh CI build because the optional `package-lock.json*` glob silently passes the copy step. Fixed in both Dockerfiles by switching `npm ci` → `npm install` in commit `007565f`. This is the cheap fix; the proper fix (refactor each Dockerfile to build from the repo-root context and copy the root lockfile into the workspace) is logged as `../../TASKS.md` P3 follow-up.
- **SPA frontend had an absolute-localhost API fallback baked into the production bundle** — `packages/web/src/api/client.js` defaulted to `http://localhost:4000` when `VITE_API_URL` was unset. Vite reads env vars at **build time**, so any ECS task-level env var on the web container is ignored once the bundle has been built. Masked in prod because pre-rotation users all held valid cached JWTs and never exercised the fresh-login path; exposed the moment JWT rotation invalidated every session and fresh-login POSTs went to `http://localhost:4000`. Fixed in commit `e1e68db` — fallback is now `/api` (relative), which the web container's nginx proxies to the api container in prod and the dev server's proxy config handles in local dev. The `VITE_API_URL` build-time override is retained for staging / edge cases.

See `C:\Users\mark\Claude Cowork\Other\Admin\WORKFLOW.md` → "Latent bugs exposed during Tier 3 migrations" and "Pre-flight checklist for any Tier 3 migration on a containerised stack" for the generalised patterns and 6-command pre-flight checklist to apply to the remaining Tier 3 migrations (Pro-curo Licence Server, Pro-curo V5).

**Where the former Appendix A/B runbooks live now:** both were moved to `C:\Users\mark\Claude Cowork\Other\Admin\WORKFLOW.md` on 2026-06-24 to keep operational runbooks in one place — the inline ECS rollback CLI under "ECS service rollback (inline CLI)", and the atomic RDS-rotation / Secrets-Manager ordering sequence under "Atomic secret rotation sequence (RDS password + JWT) on a containerised stack". (A former plaintext RDS password that appeared in Appendix B's execution record was dropped in the move; it remains only in this file's pre-consolidation git history.)